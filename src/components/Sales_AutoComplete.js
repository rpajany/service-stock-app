...
export const SalesAutoComplete = ({ Invoice_Number, inputName, txt_css, setRowItems }) => {
    // ... start update API ....
    const { selectedApi } = useContext(ApiContext);
    const BASE_URL = selectedApi;

    const { salesList, addToSales } = useSales(); // get from salesContext
    ...
    const handleItemClick = (item) => {
        ...
        addToSales(dataNew); // add to sales contex
